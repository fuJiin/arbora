"""Tests for EntorhinalCortex — fixed random projection, forward + reverse."""

import numpy as np
import pytest

from arbora.hippocampus import EntorhinalCortex


class TestInit:
    def test_shapes(self):
        ec = EntorhinalCortex(input_dim=100, output_dim=500, sparsity=0.02, seed=0)
    ...
    def test_k_rounds_to_at_least_one(self):
        ...
        ec = EntorhinalCortex(input_dim=10, output_dim=10, sparsity=0.01, seed=0)
        assert ec.k == 1

    def test_invalid_dims_raise(self):
        with pytest.raises(ValueError):
            EntorhinalCortex(input_dim=0, output_dim=10)
        with pytest.raises(ValueError):
            EntorhinalCortex(input_dim=10, output_dim=0)

    def test_invalid_sparsity_raises(self):
        with pytest.raises(ValueError):
            EntorhinalCortex(input_dim=10, output_dim=10, sparsity=0.0)
        with pytest.raises(ValueError):
            EntorhinalCortex(input_dim=10, output_dim=10, sparsity=1.5)

    def test_forward_and_reverse_are_independent_matrices(self):
        # Reverse must not be forward.T — the two pathways are independent.
        ec = EntorhinalCortex(input_dim=100, output_dim=500, seed=0)
    ...
    def test_different_seeds_produce_different_projections(self):
        ec0 = EntorhinalCortex(input_dim=100, output_dim=500, seed=0)
        ec1 = EntorhinalCortex(input_dim=100, output_dim=500, seed=1)
...
class TestDistancePreservation:
    ...
    def test_jaccard_separates_similar_from_dissimilar(self):
        ...
        ec = EntorhinalCortex(input_dim=200, output_dim=1000, sparsity=0.02, seed=0)
