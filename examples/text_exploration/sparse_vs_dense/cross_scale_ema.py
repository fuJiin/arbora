#!/usr/bin/env python3
"""ARB-139: cross-scale test of single-table SSH with EMA readout.

Configuration:
- single_table=True (validated as competitive at 5M; cleaner)
- modulate=True (already shown necessary)
- ema_alpha=0.01 (slow EMA — best 100k smoke-test result)
- subtract_mean=False (smoke test showed it hurts when combined with EMA)
- decay=0 (decay needs scale-tuning that we can't get right; EMA is our
  stability primitive of choice)

Scales: 100k, 500k, 1M, 5M, 10M. seed=0. Resumable via CSV check.

Output: data/runs/arb139/cross_scale_ema.csv

Total wall-clock estimate (no parallel contention): ~3 hours.
"""
...
def run_one(*, n_tokens: int, vocab_size: int, seed: int, ema_alpha: float) -> dict:
    print(
        f"\n--- single-table+EMA(alpha={ema_alpha:g})  "
...
def main() -> None:
    ...
    p.add_argument(
        "--csv",
        type=str,
        default="data/runs/arb139/cross_scale_ema.csv",
    )
    args = p.parse_args()
