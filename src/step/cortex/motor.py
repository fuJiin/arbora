...
class MotorRegion(SensoryRegion):
    ...
    def observe_token(self, token_id: int) -> None:
        ...
        The cached map uses selectivity (count / global_freq) rather
    ...
    def get_output(self) -> tuple[int, float]:
        ...
        """
        # Find highest-scoring column above threshold
        above = self.output_scores >= self.output_threshold
        if not above.any():
            return (-1, 0.0)

        # Among above-threshold columns, pick the highest scoring
