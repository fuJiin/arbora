"""Run loops and signal propagation for the Topology system.

Extracted from topology.py (Phase 2 of STEP-28). Contains the
TopologyRunner mixin with run loops, signal helpers, motor/reward
helpers, and logging. Methods expect ``self`` to carry Topology's
instance state (_regions, _connections, _encoder, etc.).
"""
...
if TYPE_CHECKING:
    from step.cortex.topology_types import Encoder
    from step.probes.bpc import BPCProbe
    from step.probes.centroid_bpc import CentroidBPCProbe


class TopologyRunner:
    """Mixin providing run loops for Topology.

    Methods expect self to have Topology's instance state
    (_regions, _connections, _encoder, etc.).
    """

    # Type hints so editors/linters understand the mixin contract.
    # Actual attributes live on Topology.__init__.
    ...
    _eom_steps: int
    _total_steps: int
    _reward_source: object | None
    _bg_ctx_buffer: np.ndarray | None
    force_gate_open: bool
    _ff_conns: dict[str, list[Connection]]
    _ff_buffers: dict[str, np.ndarray]
    _topo_cache: list[str] | None
    _finalized: bool
    _enable_timeline: bool
    _timeline_interval: int
    _diagnostics_interval: int
    _decoder_perm_decay: float
