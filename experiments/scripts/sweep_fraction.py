...
def run_one(fraction: float, window: int, train_cache, eval_cache) -> tuple[float, float]:
    enc_cfg = EncoderConfig(
        model_name="gpt2", n=2048, k=40, vocab_size=10000,
        adaptive=True, context_fraction=fraction,
    )
...
def main():
    ...
    for frac in fractions:
        for w in windows:
            ...
            print(f"  frac={frac:.1f} w={w:2d}: acc={acc:.1%} iou={iou:.4f} ({elapsed:.0f}s)")
