...
from step.model import predict
...
def run_diagnosis(label, enc_cfg, window, train_cache, eval_cache):
    ...
    eval_tc = TrainingConfig(
        ...
        max_tokens=EVAL_TOKENS, log_interval=50_000,
    )
    pretrain_cfg = ExperimentConfig(
        encoder=enc_cfg, model=model_cfg, training=train_tc, name="diag",
    )
    ...
    decoded_is_same_as_correct = []  # sanity

    from step.experiment import _make_stream

    eval_cfg = ExperimentConfig(
        encoder=enc_cfg, model=model_cfg, training=eval_tc, name="diag",
    )
    ...
    for t, token_id, _sdr in _make_stream(eval_cfg, eval_cache):
        ...
        if t > 0:
            ...
            if predicted_token == token_id:
                ...
                n_conf = 0
                correct_rank = 1
                for tid, tidx in model._token_id_to_idx.items():
                    tok_sdr = set()
                    for bit, idxs in model._inverted_index.items():
                        if tidx in idxs:
                            tok_sdr.add(bit)
    ...
    if iou_correct_when_wrong:
        ...
        n_decoded_wins = sum(
            1 for d, c in zip(iou_decoded_when_wrong, iou_correct_when_wrong)
            if d > c
        )
        n_tie = sum(
            1 for d, c in zip(iou_decoded_when_wrong, iou_correct_when_wrong)
        ...
        print(f"  Decoded IoU > Correct IoU: {n_decoded_wins}/{wrong_count} "
              f"({n_decoded_wins/wrong_count:.1%})")
        print(f"  Decoded IoU = Correct IoU: {n_tie}/{wrong_count} "
