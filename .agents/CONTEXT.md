...
### Class hierarchy
- **CorticalRegion** (`region.py`): ff_weights, process(), _learn_ff(), reconstruct(), efference copy, dendritic segments, L4/L2/3 pipeline
...
- **Efference copy**: When M1 speaks during `force_gate_open`, its output encoding is set on S1 via `set_efference_copy()`. S1's next `process()` subtracts predicted drive from actual drive — suppresses expected input, passes only prediction error. Breaks autoregressive fixed point.
...
- Previous: degenerate `eeeee` loops due to autoregressive fixed point
- Neural adaptation tried and reverted (inconsistent — no other region uses it)
- **Efference copy implemented** (b4522a3) — untested in REPL, needs qualitative evaluation
...
## Next Steps (Active)
- [ ] **Test efference copy in REPL** — load `personachat_k4_100k` checkpoint, evaluate M1 generation quality
- [ ] **Optimize S2 + apical feedback** — make top-down signals more useful for S1 prediction
- [ ] **M1 recurrent sequence generation** — if efference copy alone isn't enough, add L2/3 lateral dynamics for sequential state evolution
